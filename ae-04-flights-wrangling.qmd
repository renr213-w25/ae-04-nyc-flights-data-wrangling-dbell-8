...
The `flights` data frame has [**336,776 rows**.]{.underline} Each row represent a different flight departing from New York City.
...
The code below is used to filter for destinations going to RDU and GSO in the flight dataset, while not creating a new datafame. The code is also filtering any delays to arrivals and departures to be less then 0.
